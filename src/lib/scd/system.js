...
import {toBytes32, methodSig} from './utils/helpers';
...
export class System {
  ...
  */
  ...
  transferOwnership(cup, from) {
    ...
    if (!contract || ! defty) console.error('Failed to load Contracts', contract)
  ...
  proveOwnership(cup, from) {
    ...
    if (!contract) console.error('Failed to load DeftyWrap', contract)
